...
if (!class_exists('MultiPostThumbnails')) {
	...
	class MultiPostThumbnails {
		...
		public function enqueue_admin_scripts( $hook ) {
			...
			wp_enqueue_style( "mpt-admin-css", $this->plugins_url( 'assets/css/multi-post-thumbnails-admin.css', __FILE__ ) );
		}
		...
		private function post_thumbnail_html($thumbnail_id = null) {
			...
			$set_thumbnail_link = sprintf( $format_string, sprintf( esc_attr__( "%s festlegen" , 'multiple-post-thumbnails' ), $this->label ), esc_url($image_library_url), $this->post_type, $this->id, $url_class, $this->label, $thumbnail_id );
			$content = sprintf( $set_thumbnail_link, sprintf( esc_html__( "%s festlegen", 'multiple-post-thumbnails' ), $this->label ) );
